...
| G5 (PPC 970) Tiger | pmacg5 (PowerMac11,2, 2.3 GHz dual) | ✅ M3 (userland + build) | v0.2.1 G3 tarball runs unmodified on pmacg5: full smoke including `os/spawn`, PEG, fibers, marshal, `int/s64`.  Native build pipeline also works (`TIGER_HOST=pmacg5 scripts/build-tiger.sh`) — the leopard.sh 1.27.0-era `tools/patch-header.janet` Bus error does not reproduce on our pinned SHA + toolchain.  For the default `TIGER_ARCH=g3` (no-`-mcpu`, generic-powerpc) build, G3-built and G5-built binaries are byte-identical: same gcc-4.9.4 prebuilt on both hosts + no host-specific tuning flags = `bin/janet`, `libjanet.dylib`, `libMacportsLegacySupport.dylib`, `janet.h` all bit-for-bit identical; `libjanet.a` differs only in `ar` archive timestamps.  Bench: **1.701 s total** best-of-5, **~1.93× faster than 1.42 GHz G4** on the same Janet binary (~19% IPC win on top of the clock advantage).  See [session 010](docs/sessions/010-m3-g5-bootstrap/). |
